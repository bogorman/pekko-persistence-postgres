...
import pekko.persistence.postgres.config._
...
class PekkoPersistenceConfigTest extends AnyFlatSpec with Matchers with OptionValues {
  val config: Config = ConfigFactory.parseString("""
      |postgres-journal {
      |  class = "pekko.persistence.postgres.journal.PostgresAsyncWriteJournal"
      |
      |  tables {
      |    journal {
      |      partitions {
      ...
      |  }
      |
      |  dao = "pekko.persistence.postgres.journal.dao.FlatJournalDao"
      |
      |  logicalDelete = true
      |
      |  tags {
      ...
      |  class = "pekko.persistence.postgres.snapshot.PostgresSnapshotStore"
      |
      |  tables {
      |    snapshot {
      |      tableName = "snapshot"
      ...
      |  }
      |
      |  dao = "pekko.persistence.postgres.snapshot.dao.ByteArraySnapshotDao"
      |
      |  slick {
      ...
      |  class = "pekko.persistence.postgres.query.PostgresReadJournalProvider"
      |
      ...
      |  max-buffer-size = "10"
      |
      |  dao = "pekko.persistence.postgres.query.dao.FlatReadJournalDao"
      |
      |  tags {
      |    cacheTtl = 12 hours
      |  }
  ...
  "empty config" should "parse JournalConfig" in {
    ...
    cfg.pluginConfig.dao shouldBe "pekko.persistence.postgres.journal.dao.FlatJournalDao"
  ...
  it should "parse SnapshotConfig" in {
    ...
    cfg.pluginConfig.dao shouldBe "pekko.persistence.postgres.snapshot.dao.ByteArraySnapshotDao"
  ...
  it should "parse ReadJournalConfig" in {
    ...
    cfg.pluginConfig.dao shouldBe "pekko.persistence.postgres.query.dao.FlatReadJournalDao"
