...
package pekko.persistence.postgres.config
...
class JournalPluginConfig(config: Config) {
  val dao: String = config.asString("dao", "pekko.persistence.postgres.journal.dao.FlatJournalDao")
...
class ReadJournalPluginConfig(config: Config) {
  val dao: String = config.as[String]("dao", "pekko.persistence.postgres.query.dao.FlatReadJournalDao")
  override def toString: String = s"ReadJournalPluginConfig($dao)"
}

class SnapshotPluginConfig(config: Config) {
  val dao: String = config.as[String]("dao", "pekko.persistence.postgres.snapshot.dao.ByteArraySnapshotDao")
